...
```sql:line-numbers
create table WRKWVS.STKL_20_WE_REGIO_ALLE as
select * from connection to snow(
select DISTINCT A.*, B.KAL_TAG_ID, B.K_LIEF_ID AS LIEF_ID, B.P_LIFEINH, B.P_LIFMNG, b.P_AENDDAT, b.P_AENDZEIT, B.LAGNR, B.MA_LAG_ID
from
(select DISTINCT AA.KOPF_ART_ID, AA.POS_ART_ID
from LEGACY_DWH.DWH.F_STUECKLISTE AA
join LEGACY_DWH.DWH.LU_D_MA_LAG BB
on AA.MA_LAG_ID = BB.MA_LAG_ID
where AA.STKL_TYP_ID = 20
)
A
join LEGACY_DWH.DWH.F_ELVS_BO B
on a.POS_ART_ID = B.P_NAN_ART_ID
WHERE B.KAL_TAG_ID > dateadd(day, -30 , CURRENT_DATE)
AND B.P_LIFEINH <> 0 AND B.P_LIFMNG <> 0 AND B.K_LIEF_ID NOT IN ('','*')
ORDER BY B.KAL_TAG_ID, A.KOPF_ART_ID, B.P_LIFEINH, B.P_LIFMNG desc
)
```
